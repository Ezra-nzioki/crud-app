...
router.post('/signup', async (req, res) => {
  try {
    ...
    res.status(201).json({ user: { id: user._id, name: user.name, email: user.email, age: user.age }, token })
  } catch (error) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id, email: user.email }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' })
    res.status(200).json({ user: { id: user._id, name: user.name, email: user.email, age: user.age }, token })
  } catch (error) {
    res.status(500).json({ message: error.message })
  }
})
